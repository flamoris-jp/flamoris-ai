...
- Applications may call the Intelligence Hub directly when no persistent Agent is needed.
...
1. **One authority per domain**
   ...
   - Intelligence execution/routing belongs to the Intelligence Hub.
...
ここ自体を巨大なAIランタイムにするのではなく、AI Agent、推論・Coding系のIntelligence Hub、画像・動画・音楽・音声生成のGeneration MCPについて、役割と境界、共通方針を整理します。
...
- LLMやCoding Agentのprovider選択・実行はIntelligence Hubが持つ。
